...
bool ImGui::TempInputScalar(const ImRect& bb, ImGuiID id, const char* label, ImGuiDataType data_type, void* p_data, const char* format, const void* p_clamp_min, const void* p_clamp_max)
{
    ...
    ImGuiInputTextFlags flags =
        static_cast<int>(ImGuiInputTextFlags_AutoSelectAll) |
        static_cast<int>(ImGuiInputTextFlags_NoMarkEdited);
...
bool ImGui::CollapsingHeader(const char* label, bool* p_visible, ImGuiTreeNodeFlags flags)
{
    ...
    if (p_visible)
      flags |= static_cast<int>(ImGuiTreeNodeFlags_AllowItemOverlap) |
               static_cast<int>(ImGuiTreeNodeFlags_ClipLabelForTrailingButton);
...
// tab_map_item was created, so we clear them regardless.
...
void ImGui::TabBarQueueFocus(ImGuiTabBar* tab_bar,const ImGuiTabItem* tab) {
...
bool ImGui::BeginTabItem(const char* label, bool* p_open,
  ...
  if (ret) {
    ...
    tab_map_item->second.frame =
        ImLerp(tab_map_item->second.frame,
               tab_bar->SelectedTabId == tab->ID ? 1.f : 0.f,
               std::min(1.0f, g.IO.DeltaTime * 3.5f));

    PushStyleVar(ImGuiStyleVar_Alpha, tab_map_item->second.frame);
    PushStyleColor(ImGuiCol_Text, GetColorU32(ImGuiCol_Text));

    if (!(flags & ImGuiTabItemFlags_NoPushId)) PushOverrideID(tab->ID);
  }
...
bool ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open,
  ...
  tab_map_item->second.text = ImLerp(
      ...
      tab_bar->SelectedTabId == id
          ? (ImVec4)(ImColor)GetColorU32(ImGuiCol_Border)
